# 这个是25号搞1000test的时候直接复制过来的基本啥也没改的24早上版本
import numpy as np
import torch
...
class EDPSPHead(PSPHead):
        ...
        if mode in ('stage2', 'test', 'stage3'):
            ...
            if self.with_error_ds:
                ...
                # elif mode == 'stage3':
                    ...
                    # error_mask = torch.where(error_logit>0.5, error_logit, torch.full_like(error_logit, 0.))
                    # output = torch.mul(error_mask, output) + coarse_output
                    output = torch.mul(error_logit, output) + coarse_output

                    # self.total_num = torch.ones_like(error_mask).sum()
                    # self.activated_num = error_mask.clone().detach().sum()
                    # self.activated_rate = self.activated_num/self.total_num
                    # self.detected_error =
    ...
    def losses(self, seg_logit, coarse_logit, seg_label, error_map=None):
        ...
        loss['acc_seg_coarse'] = accuracy(coarse_logit, seg_label)
        # if self.with_error_ds:
        #     loss['error_loss_seg'], loss['acc_error_pred'], loss['GT_error_rate'] = accuracy_error_loss(coarse_logit,
        #                                                     seg_label,
        #                                                     error_logit)
        #     #print(s==loss['acc_seg_coarse'])
        #     # loss['error_loss_seg'] *= 10
        #     # loss['acc_error_pred'] = self.eds_correct_rate
        #     loss['av_num'] = self.activated_num
        #     loss['total_num'] = self.total_num
        #     loss['P_error_rate'] = self.activated_rate
        return loss
